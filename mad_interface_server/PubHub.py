...
'''
    ...
    Abstract:
        ...
        Establish communication with client side6, and server will do some actions
...
def discovery():
    ...
    information will response HTTP Link Header and response body to subscribers.
    ...
    dt = information.determine_topic_and_hub(pos_id, pos_type)
...
def hub():
    ...
    information will be a Hub to deal with subscribe/unsubcribe action.
...
def validate_topic_url(postData):
    ...
    is_find_url = information.match_url(postInfo['hub.topic'])
    # if answer.judge:
    if is_find_url == True:
        ...
        if str(req.status_code)[:1] == '2' and str(req.content) == str(randomKey):
            information.store_subscriber(postInfo['hub.topic'],postInfo['hub.callback'])
            information.content_distribution(postInfo['hub.callback'])
            print 'success'
        else:
            print 'fail'
    else:
