...
function getPosInfo(){
    ...
    for(var i=0; i<25; i++){
        ...
        posObject.lat = Math.random() * 0.11 + 37.708;
        posObject.lng = Math.random() * 0.06 - 122.507;
        alert(posObject.lat+","+posObject.lng);
        posObject.ward = generateCode(posObject.lat,posObject.lng);
        //posObject.special_id = "USA-"+generateCode(posObject.lat,posObject.lng);+"-"+random();
        posGroup.push(posObject);
        /*posObject.ward = xmlDoc.getElementsByTagName("District")[i].childNodes[0].nodeValue;
        posObj.special_id = xmlDoc.getElementsByTagName("ID")[i].childNodes[0].nodeValue;
        posObj.lat = xmlDoc.getElementsByTagName("Latitude")[i].childNodes[0].nodeValue;
        posObj.lng = xmlDoc.getElementsByTagName("Longitude")[i].childNodes[0].nodeValue;
        pos.push(posObj);*/
